...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8000";
 ...
 function ExamsPageContent() {
  ...
  const [selectedProviders, setSelectedProviders] = useState([]); // Changed to array for multiple selection
  ...
  const updateURL = (newProviders, newCategories, newKeyword) => {
    ...
    const newUrl = `/exams${params.toString() ? `?${params.toString()}` : ""}`;
    router.replace(newUrl, { scroll: false });
  };

  // Initialize from URL params if available (only on first mount)
  useEffect(() => {
                ...
                {filteredExams.map((exam) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      asChild
                    >
                      <Link href={`/exam-details/${exam.slug || createSlug(exam.code)}`}>
                        Start Practicing
